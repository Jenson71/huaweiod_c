...
static void InOrder(const Trnode* root,int* time, void (*pfun)(Item item,int* time));
...
void Traverse(const Tree* ptree, int* time,void (*pfun)(Item item,int* time))
{

    if (ptree != NULL)
        InOrder(ptree->root,time, pfun);
}
...
static void InOrder(const Trnode* root,int* time, void (*pfun)(Item item,int* time))
{
    if (root != NULL)
    {
        InOrder(root->left,time, pfun);
        (*pfun)(root->item, time);
        InOrder(root->right,time, pfun);
    }
}
